...
结构与文件 → 视觉/坐标 → 标签语义 → 候选与目标 → 泄漏/PII → 去重与整组 split → 配额/分布。构建时执行自动化全量校验和分层抽样报告；无法修复的行按明确规则剔除，并在迭代记录中报告重建前后数量，不新增人工标注。
